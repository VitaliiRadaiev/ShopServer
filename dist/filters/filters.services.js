"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.FiltersServices = void 0;
const inversify_1 = require("inversify");
require("reflect-metadata");
const types_1 = require("../types");
const prisma_service_1 = require("../database/prisma.service");
let FiltersServices = class FiltersServices {
    constructor(configService, prismaService) {
        this.configService = configService;
        this.prismaService = prismaService;
    }
    getFilters() {
        return __awaiter(this, void 0, void 0, function* () {
            return this.prismaService.client.filterBlockModel.findMany({
                include: {
                    items: true
                }
            });
        });
    }
    createFilter(categoryId, title) {
        return __awaiter(this, void 0, void 0, function* () {
            return this.prismaService.client.filterBlockModel.create({
                data: {
                    title,
                    categoryId
                }
            });
        });
    }
    updateFilter(id, title) {
        return __awaiter(this, void 0, void 0, function* () {
            return this.prismaService.client.filterBlockModel.update({
                where: { id },
                data: {
                    title
                }
            });
        });
    }
    deleteFilter(id) {
        return __awaiter(this, void 0, void 0, function* () {
            const deleted = yield this.prismaService.client.filterBlockModel.delete({
                where: { id }
            });
            return !!deleted;
        });
    }
    createFilterItem(filterId, title) {
        return __awaiter(this, void 0, void 0, function* () {
            return this.prismaService.client.filterItemModel.create({
                data: {
                    filterBlockId: filterId,
                    title
                }
            });
        });
    }
    updateFilterItem(id, title) {
        return __awaiter(this, void 0, void 0, function* () {
            return this.prismaService.client.filterItemModel.update({
                where: { id },
                data: {
                    title
                }
            });
        });
    }
    deleteFilterItem(id) {
        return __awaiter(this, void 0, void 0, function* () {
            const deleted = yield this.prismaService.client.filterItemModel.delete({
                where: { id }
            });
            return !!deleted;
        });
    }
};
exports.FiltersServices = FiltersServices;
exports.FiltersServices = FiltersServices = __decorate([
    (0, inversify_1.injectable)(),
    __param(0, (0, inversify_1.inject)(types_1.TYPES.ConfigService)),
    __param(1, (0, inversify_1.inject)(types_1.TYPES.PrismaService)),
    __metadata("design:paramtypes", [Object, prisma_service_1.PrismaService])
], FiltersServices);
//# sourceMappingURL=filters.services.js.map